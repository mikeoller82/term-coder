...
class ContextEngine:
    ...
    def select_context(self, files: Optional[List[str]] = None, query: Optional[str] = None, budget_tokens: int = 8000) -> ContextSelection:
        ...
        # 2b) Recent files get a small boost
        recent = set(self.session.get_recent_files(limit=20))
        for cf in selected:
            if cf.path in recent:
                cf.relevance_score = max(cf.relevance_score, cf.relevance_score + 0.05)
        ...
        for cf in selected:
            # Prefer model-specific tokenizer if available
            try:
                text = (root / cf.path).read_text(errors="ignore")
            except Exception:
                text = ""
            est = estimator.estimate(text)
            file_tokens = max(est, self._estimate_tokens_for_file(root, cf.path))
